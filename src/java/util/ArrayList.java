 ...
 * 构造函数
     * 无参: 底层默认创建一个空数组,在进行第一次添加操作时进行扩容为默认的容量10
     * 传入int n: 创建一个容h量为n的数组
     * 传入集合: 使用Arrays.copyOf()进行遍历赋值到当前ArrayList中
 * 底层数组设定的最大长度为MAX_ARRAY_SIZE = Integer.MAX_VALUE - 8
 ...
 * 序列化：
     * 只序列化了被使用的数据
 *
 *  1. 构造一个数目为0的数组，elements会引用EMPTY_ELEMENTDATA或者DEFAULTCAPACITY_EMPTY_ELEMENTDATA
 */

/**
 * 基于动态数组实现，支持随机访问,克隆，序列化，可迭代
 */
        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     * 获取list需要的size，如果是空数组则返回最小的容量，否则返回minCapacity
     */
    ...
    /**
     * 保证数组不越界实际工作的函数，minCapacity大于实际长度则扩容
     */
    ...
    private static int hugeCapacity(int minCapacity) {
        ...
        // 大于MAX_ARRAY_SIZE返回int的最大值，否则返回MAX_ARRAY_SIZE返
